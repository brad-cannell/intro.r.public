...
This week, we will practice using for loops and performing repeated operations with functions from the purrr package. We will do so using a couple of different data sets:
...
Please download Class Survey 5141.csv, import it into your R global environment as class_survey.
...
Task 2. Please look at the codebook for the class_survey data. Several questions on the survey included Not Sure as a response option. They were questions 2, 4, 6, 7, 9, 10, 13, 14, 15, 16, 17, 18, 20, and 26. Please rename each of the columns contains these questions as required, still_enroll, package, genhealth, asthma, seatbelt, insurance, persdoc, hispanic, race, veteran, marital, employment, sex, and color.
...
```{r}
cols_9 <- c("required", "still_enroll", "package", "genhealth", "asthma", 
            "seatbelt", "insurance", "persdoc", "hispanic", "race", "veteran",
            "marital", "employment", "sex", "color")
```
...
1. Which function can we use to apply other functions (e.g., str_to_lower) to some, or all, of our column names simultaneously in order to change them in some way?

* We can use the rename_with() function to apply other functions (e.g., str_to_lower) to some, or all, of our column names simultaneously in order to change them in some way. We can apply functions to individual column names inside of rename() and select(), but we can apply those functions to some, or all, of our column names simultaneously inside of rename() and select(). 
...
```{r}
for (i in seq_along(cols_9)) {
  class_survey[[cols_9[[i]]]] <- as.numeric(class_survey[[cols_9[[i]]]])
}
```
...
2. Tasks 4 through 7 above essentially required you to do the same thing in four different slightly different ways. For this particular task, which method did you prefer, and why?
...
3. Which column in the class_survey data frame contains the greatest number of Not Sure responses?
...
4. Which column in the class_survey data frame contains the greatest number of missing values?
...
5. How many people reported that they have ever been told by a doctor, nurse, or other health professional that they have asthma?
