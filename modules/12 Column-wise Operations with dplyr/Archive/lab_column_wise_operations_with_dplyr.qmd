...
In this lab, we will practice writing custom R functions to reduce repetition and improve code clarity. Writing functions is a critical step in moving from one-off scripts to reusable, readable, and maintainable analytic code.

Using a class survey dataset, we will:

- Identify repeated operations in our code

- Write functions that generalize those operations

- Apply functions to real data

- Reflect on how functions improve workflow efficiency

# Data for This Lab

We will use the class survey dataset introduced earlier in the course.
...
* [Click here to view the codebook for the class survey data](https://www.dropbox.com/s/kjyfq8cfur0hzdp/5313%20-%205314%20Day%20One%20Survey%20Codebook.pdf?dl=0). Review the associated codebook to understand variable coding and response options.

Several survey questions include **Not Sure** as a response option, which will serve as the motivating example for writing functions.
...
```{r}
class_survey <- read_rds("Data/class_survey.Rds")
```

# Task 2

Please look at the codebook for the **class_survey** data. Several questions on the survey included a "Not Sure" response option coded as a "9". They were questions 2, 4, 6, 7, 9, 10, 13, 14, 15, 16, 17, 18, 20, and 26. 

Before analyzing this data, we will convert all of the "9" values to "NA" values. However, the very first thing we want to do is give each of those columns more informative names. Specifically, we want to rename those columns: 
...
We have already learned how to rename columns using the `rename()` function. 
...
We can also rename multiple columns at once by creating a character vector of column names and applying them using the `rename_with()` function instead. Using this method has some advantages. 
...
However, the code above is more tedious to write, more difficult to maintain, more error-prone, and less concise than it needs to be. Instead, let's coerce all of the columns with a "Not Sure" "Not Sure" response option coded as a "9" from character type to numeric type using the `across()` function to help remove some of the repetition from the code.

## Question 3

Which of the following code chunks will correctly coerce all of the columns with a "Not Sure" response option coded as a "9" from character type to numeric type using the `across()` function?
