---
title: "Lab: Introduction to Bivariate Descriptive Analysis in R"
author: Brad Cannell
...
* [Click here to view/download the WHAS 500 data codebook](https://tcu.box.com/s/jcoowjwssvp84u1cnd3l3iz48qcrn5tl). Look over the codebook to learn a little bit about the data you will be working with.

* [Click here to view/download the WHAS 500 data](https://tcu.box.com/s/vy7ita9hsrm1e312ses9tlm47w27dgyd).
...
```{r}
whas500 <- read_table("/Users/bradcannell/Library/CloudStorage/Box-Box/Datasets/WHAS/whas500.txt", 
  col_types = cols(
    id        = col_character(), 
    age       = col_integer(), 
    gender    = col_integer(), 
...
Create a two-way frequency table for cohort year by history of cardiovascular disease.

```{r}
year_by_cvd <- whas500 |> 
  # Drop missing values
  filter(!is.na(year), !is.na(cvd)) |> 
  
  # Calculate the joint counts
  count(year, cvd, .drop = FALSE) |> 
  
  # Add overall counts and proportions
  mutate(
    n_overall    = sum(n),
    prop_overall = n / n_overall
  ) |> 

  # Add row counts and row proportions
  group_by(year) |>
  mutate(
    n_row = sum(n),
    prop_row  = n / n_row
...
Optionally, filter the results to make them easier to read:
...
Create a two-way frequency table for history of cardiovascular disease by cohort year.

```{r}
cvd_by_year <- whas500 |> 
  # Drop missing values
  filter(!is.na(cvd), !is.na(year)) |> 
  
  # Calculate the joint counts
  count(cvd, year, .drop = FALSE) |> 
